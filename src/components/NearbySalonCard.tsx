...
const NearbySalonCard = ({ salon }: NearbySalonCardProps) => {
  ...
  return (
    <div
      ...
      className="flex-shrink-0 w-[200px] bg-card rounded-2xl overflow-hidden card-shadow border border-border transition-all duration-200 active:scale-[0.97] cursor-pointer"
    >
      <div className="relative h-[130px]">
        <img src={salon.image} alt={salon.name} className="w-full h-full object-cover" loading="lazy" />
        <span className={`absolute top-2.5 right-2.5 text-[10px] font-heading font-semibold px-2 py-0.5 rounded-lg backdrop-blur-sm ${
          salon.isOpen
      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between mt-2.5">
          ...
          <button
            ...
            className="text-[11px] font-heading font-semibold text-primary-foreground bg-primary px-3.5 py-1.5 rounded-lg active:scale-95 transition-transform min-h-[32px]"
          >
            Book
          </button>
        </div>
